...
static int usage(char **argv) {
	printf("nUsage:t%s -q -f config_file program_name [arguments]n"
	       "t-q makes proxychains quiet - this overrides the config settingn"
	       "t-f allows to manually specify a configfile to usen"
	       "tfor example : proxychains telnet somehost.comn" "More help in README filenn", argv[0]);
	return EXIT_FAILURE;
}
...
int main(int argc, char *argv[]) {
	...
	if(!quiet)
		fprintf(stderr, LOG_PREFIX "config file found: %sn", path);
	...
	if(!prefix) {
		fprintf(stderr, "couldnt locate %sn", dll_name);
		return EXIT_FAILURE;
	}

	if(!quiet)
		fprintf(stderr, LOG_PREFIX "preloading %s/%sn", prefix, dll_name);
